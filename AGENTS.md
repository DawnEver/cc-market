...
Community marketplace of Claude Code **and Codex** plugins. Each plugin lives in its own directory with independent CLAUDE.md, AGENTS.md, and `.claude/rules/`. Plugins are authored against the Claude Code format (source of truth); the Codex artifacts (`.codex-plugin/plugin.json` + `.agents/plugins/marketplace.json`) are generated by `scripts/gen-codex.mjs`. See `CODEX-SUPPORT.md`.
