...
import {
  inferModule, inferCategory, collectMemoryRefs, crossReferenceFindings, writeBackMemoryRefs, findingToMemoryEntry
} from '../lib.mjs';
...
const RESOLVED_FILE = join(SHARP_REVIEW_DIR, 'resolved.txt');
...
function main() {
  ...
  // Cross-reference with memory entries (uses lib.mjs O(1) inverted index)
  const { refs: memoryRefs, idIndex: memoryIdIndex } = collectMemoryRefs();
  crossReferenceFindings(allFindings, memoryRefs, memoryIdIndex);

  // Create individual memory entries for HIGH/MEDIUM findings (rem-managed lifecycle)
  let memCreated = 0;
  for (const f of allFindings) {
    if (!f.memoryRef) {
      const relPath = findingToMemoryEntry(f);
      if (relPath) { f.memoryRef = relPath; memCreated++; }
    }
  }
  if (memCreated > 0) console.log(`[sync-tasks] ${memCreated} findings → memory entries`);

  // Write SR-IDs back to matched/created memory files for bidirectional linking
