...
- If `report.watch.version_tracking.enabled` and `report.components.git_version.data.new_commits > 0`:
  1. The `deploy` action (worktree-based test gate) is in `report.anomalies[].remedy_plan` if `new_version_available` exists
  2. Execute the deploy action. On pass → server restarts, known-good updated. On fail → rejected.
