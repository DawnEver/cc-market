...
`scripts/daemon/trigger-watch.py` is a standalone Python process that polls `trigger.json`
independently of any Claude Code session:

```bash
# In a separate terminal (survives session restarts):
python ${CLAUDE_PLUGIN_ROOT}/scripts/daemon/trigger-watch.py --project-dir .
```

On trigger change it runs `scripts/cli/watch.py` (the full AI check loop) directly — same venv,
...
- **trigger-watch.py** — daemon-driven, session-independent base layer: watchd writes
  `trigger.json` on anomaly → trigger-watch runs the AI check (`scripts/cli/watch.py`)
...
- **Monitor + trigger-emit.py** — in-session real-time layer: while a live `/watch:watch`
  session is alive, it arms a `Monitor` whose command is `scripts/cli/trigger-emit.py`
...
`watchd` daemon itself (`watchd.interval`). Reacting twice is harmless — `scripts/cli/watch.py`
remedies are idempotent.

### Process-identity note (cross-platform)

`trigger-watch.py` and `daemon.py` call `bootstrap.ensure()`, which re-execs into the
...
When `trigger-watch.py` handles a trigger it runs `scripts/cli/watch.py` (the full check loop),
