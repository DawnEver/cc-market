...
export async function openRemoteSession(opts) {
  const { host, port, token, provider, model, write, project, profile } = opts;
  if (!provider) throw new Error("openRemoteSession: provider is required");
  const conn = await connectNode({ host, port, token });
  try {
    const desc = await conn.request("node/spawn", { provider, model, write: !!write, project, profile: profile ?? null });
    return {
      id: desc.id,
      pid: desc.pid ?? null,
